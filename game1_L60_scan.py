...
SV_MIN = 1e-10
OUTPUT_DIR = "data"
OUTPUT_FILE = os.path.join(OUTPUT_DIR, "game1_L60_log.csv")

print("=" * 60)
print("[游戏一·L收敛检验] 单键缺陷扫描 L=60")
print(f"参数: L={L}, U={U}")
...
print("\n" + "=" * 60)
print("[游戏一·L收敛检验] 全部完成")
print(f"数据已保存至: {os.path.abspath(OUTPUT_FILE)}")
print("=" * 60)
print("\nL收敛性分析:")
print("  缺陷 0.5t: L=40时 A=5.47 | L=60时 A=?")
print("  缺陷 1.5t: L=40时 A=0.97 | L=60时 A=?")
print("\n如果L=60的A值与L=40接近，菜单一接近成功。")
print("如果L=60的A值回归均匀链值，菜单一是有限尺寸瞬态。")